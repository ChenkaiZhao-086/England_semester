...
ci_base_3 <- future_map(
  1:length(mat_base3),
  function(i) {
    current_name <- names(mat_base3)[i]
    res <- do.Causal(
      data = prep.Causal(
        data = LTLA_gr,
      ...
      raw.data = "series",
      path = paste0(
        "outputs/3 control/3ci_m_base_gr/",
        names(mat_base3[i]),
        ".pdf"
      ),
...
# plot.it(
#   ci_base_3,
#   length.dat = mat_base3,
#   report_table = base_table_3,
#   select.length = 41,
#   title = "All matched LTLAs",
#   save = T,
#   path = "outputs/3 control/allALRI3.pdf",
#   width = 14,
#   height = 12
# )
...
ci_base_nolon_3 <- future_map(
  1:length(mat_nolondon3),
  function(i) {
    current_name <- names(mat_nolondon3)[i]
    res <- do.Causal(
      data = prep.Causal(
        data = LTLA_gr,
      ...
      raw.data = "series",
      path = paste0(
        "outputs/3 control/3ci_m_base_gr_nolon/",
        names(mat_nolondon3[i]),
        ".pdf"
      ),
      model.args = list(niter = 5000, nseasons = 7, season.duration = 1)
    )
...
# plot.it(
#   ci_base_nolon_3,
#   length.dat = mat_nolondon3,
#   report_table = ci_base_nolon_table3,
#   title = "Not include London (outside of M25)",
#   save = T,
#   path = "outputs/3 control/nolondon3.pdf",
#   width = 14,
#   height = 12
# )
...
over2_3 <- future_map(
  1:length(mat_over2hall3),
  function(i) {
    current_name <- names(mat_over2hall3)[i]
    res <- do.Causal(
      data = prep.Causal(
        data = LTLA_gr,
      ...
      raw.data = "series",
      path = paste0(
        "outputs/3 control/3ci_m_base_gr_over2/",
        names(mat_over2hall3[i]),
        ".pdf"
      ),
      model.args = list(niter = 5000, nseasons = 7, season.duration = 1)
    )
...
# plot.it(
#   over2_3,
#   length.dat = mat_over2hall3,
#   report_table = over2_table3,
#   title = "Matched in LTLA with over one hall(s)",
#   save = T,
#   path = "outputs/3 control/over2_3.pdf",
#   width = 14,
#   height = 12
# )
