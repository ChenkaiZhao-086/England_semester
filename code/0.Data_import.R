...
HEI_cam_hall_dat <- read_excel(ExcelToUse, sheet = "1.HEI-campus-hall")
HEI_cam_dat <- read_excel(ExcelToUse, sheet = "1.1HEI-campus")
Hall_LTLA_dat <- read_excel(ExcelToUse, sheet = "1.2Hall-LTLA")
LTLA_dat <- read_excel(ExcelToUse, sheet = "1.3 LTLA") %>%
  dplyr::select(!starts_with(c("HS", "BAME")))
LTLA_case_dat <- read_excel(ExcelToUse, sheet = "1.4LTLA-case-rt")
rt_dat <- read_csv("data/rt.dat.csv")

gmr <- read_csv("data/2020_GB_Region_Mobility_Report.csv") %>%
  mutate(
    Google = if_else(
      is.na(sub_region_2) == T,
      paste0(sub_region_1, "_"),
      paste(sub_region_1, sub_region_2, sep = "_")
    )
  ) %>%
  dplyr::select(
    Google,
